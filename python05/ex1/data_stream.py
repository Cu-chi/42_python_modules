...
def arg_split(input_string: str) -> tuple[str, str]:
    ...
    if dot_pos == -1 or dot_pos == i - 1:
        raise StreamDataError(f"error parsing arg '{input_string}'")
...
class SensorStream(DataStream):
    ...
    def process_batch(self, data_batch: List[Any]) -> str:
        ...
        for elem in data_batch:
            ...
            data: tuple[str, str] = arg_split(elem)
            if data[0] == "temp":
                temp += float(data[1])
                nb_temp += 1
            self.processed += 1
        ...
        result: str = f"Sensor analysis: {self.processed} reading"
        if self.processed > 1:
            result += "s"
        return result + f" processed, avg temp: {self.avg_temp:.2f}"
